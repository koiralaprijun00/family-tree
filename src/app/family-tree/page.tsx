...
const FamilyTreePage = () => {
  ...
  const [addingRelative, setAddingRelative] = useState<'parent' | 'child' | 'spouse' | null>(null);
  ...
  const handleAddRelative = (type: 'parent' | 'child' | 'spouse') => {
  ...
  const handleDeleteMember = () => {
    ...
    // Create a deep copy of the family data to avoid mutating the original
    ...
    // Handle the case where the selected member is the spouse of the root
    ...
    // Recursive function to remove the selected member
    const removeMember = (node: FamilyMember): boolean => {
      // Check if the node has children
      if (node.children) {
        // Look for the selected member in the children array
        const childIndex = node.children.findIndex(child => child.id === selectedMember.id);
        if (childIndex !== -1) {
          // If the selected member is found, remove them
          const child = node.children[childIndex];
          // If the child has a spouse, remove the spouse reference from the spouse
          if (child.spouseId) {
        ...
        // Check if the selected member is a spouse of one of the children
    ...
    // Process spouse relationships and update the state
    const processedData = processSpouseRelationships(updatedFamilyData);
    setFamilyData(processedData);
    setSelectedMember(null);
    setTreeKey(k => k + 1);
  ...
  const handleSaveProfile = (updatedMember: FamilyMember) => {
    ...
    if (selectedMember?.id === familyData.id) {
      Object.assign(updatedFamilyData, {
        ...updatedMember,
        id: familyData.id,
        children: updatedFamilyData.children,
        spouse: updatedFamilyData.spouse,
        spouseId: updatedFamilyData.spouseId
      });
    } else {
      const updateMemberRecursive = (node: FamilyMember): boolean => {
        ...
        if (node.spouse?.id === selectedMember?.id) {
          if (node.spouse) {
            Object.assign(node.spouse, {
              ...updatedMember,
              id: node.spouse.id,
              children: node.spouse?.children ?? [],
              spouse: node.spouse.spouse,
              spouseId: node.spouse.spouseId
            });
          }
          return true;
        }
        ...
        if (node.children) {
          return node.children.some(updateMemberRecursive);
        }
        
        return false;
      };
      
      updateMemberRecursive(updatedFamilyData);
    }
  ...
  const handleEditSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const updatedMember: FamilyMember = {
      ...
      // Add other fields as needed based on your FamilyMember type
    };
    handleSaveProfile(updatedMember);
  };
                ...
                <div className="bg-white p-4 rounded-lg shadow-md mb-4">
                  ...
                  {editingMember ? (
                    <form onSubmit={handleEditSubmit} className="space-y-4">
                      ...
                      {/* Add more fields here as needed based on your FamilyMember type */}
